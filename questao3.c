...
int main() {
    int n1;
    printf("quantos clientes serao registrados?");
    scanf("%d", &n1);
    int idades[n1];
    int notas[n1];
    for (int i = 0; i < n1; i++) {
        printf("Insira a idade do cliente %d (total de %d clientes): ", i + 1, n1);
        scanf("%d", &idades[i]);
        printf("Insira a nota do cliente %d (total de %d clientes)(3 - excelente, 2 - bom, 1 - regular): ", i + 1, n1);
        scanf("%d", &notas[i]);
    }

    int opiniao1 = calculo(notas, 1, n1);
    int opiniao2 = calculo(notas, 2, n1);
    int opiniao3 = calculo(notas, 3, n1);
    ...
    float media1 = media(notas, idades, n1, 1);
    float media2 = media(notas, idades, n1, 2);
    float media3 = media(notas, idades, n1, 3);
